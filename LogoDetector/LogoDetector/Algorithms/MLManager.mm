...
- (BOOL) isOceanicLogo: (MSERFeature *) feature;
{
    return YES;
}
...
- (void) loadTemplate
{
    ...
    //SaveFile Key1
    //NumberOfHoles 2
    //convexHullAreaRate = 0.391
    
    //if(feature.numberofholes == numberOfHoles
    //convexHullAreaRate
...
- (void) storeTemplate
{
    ...
    NSLog(@"Begin Logging: Oceanic Logo");
    //NSLog(@"Number of holes:  %i", (int)_logoTemplate.numberOfHoles);
    
    NSLog(@"AreaRate, MinRectArea, SkeletLength, ContourArea : %f \t %f \t %f \t %f",
          fabs(_logoTemplate.convexHullAreaRate),
          fabs(_logoTemplate.minRectAreaRate),
          fabs(_logoTemplate.skeletLengthRate),
          fabs(_logoTemplate.contourAreaRate)
          );
